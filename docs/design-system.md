...
Header product configuration lives in `apps/web/components/marketplace-navigation.ts`, while the presentation and disclosure behavior live beside it in `marketplace-header.tsx`. T11 supplies the authenticated account state, T16 supplies the authoritative cart line count and `/cart` screen, and T08–T09 supply catalogue/search data. `/login`, `/cart`, and `/search` are stable storefront destinations.
