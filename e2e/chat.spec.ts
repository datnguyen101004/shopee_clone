...
async function routeAuthenticatedChat(page: Page, withConversation = true) {
  ...
  await page.route('**/api/v1/chat/conversations/unread-count', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ chatVersion: 'chat-v1', unreadCount: withConversation ? 1 : 0 }),
    }),
  );
  await page.route('**/api/v1/chat/conversations?*', (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        chatVersion: 'chat-v1',
        items: withConversation ? [conversation] : [],
        nextCursor: null,
        unreadCount: withConversation ? 1 : 0,
      }),
    }),
  );
  await page.route(`**/api/v1/chat/conversations/${ids.conversation}/messages*`, (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
  ...
  await page.route('**/api/v1/chat/targets/shops/*', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
...
chatTest.describe('floating chat [mocked-http] journeys', () => {
  chatTest(
    'opens from the global trigger, preserves contact state, and remains accessible at every viewport',
    async ({ page, chatAccounts, singleOwnerShop }) => {
      ...
      await expect(
        page.locator('.floating-chat__message-bubble', { hasText: 'Xin chào' }),
      ).toBeVisible();
      const close = page
        .getByRole('dialog', { name: 'Trò chuyện' })
        .getByRole('button', { name: 'Đóng trò chuyện' });
  ...
  chatTest(
    'keeps a materialized conversation at the newest message after reopening and supports keyboard close',
    async ({ page }) => {
      await routeAuthenticatedChat(page);
      await page.goto('/account/profile');
  ...
  chatTest(
    'shows the empty state without creating a conversation when no history exists',
    async ({ page }) => {
  ...
  chatTest(
    'opens the same owner-targeted widget from product and public shop details',
    async ({ page, request }) => {
