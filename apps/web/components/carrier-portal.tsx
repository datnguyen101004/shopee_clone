  ...
  return (
    <main className="carrier-ui-page" aria-label="Quản lý đơn hàng">
      ...
      <section className="carrier-ui-table-card" aria-label="Danh sách đơn hàng">
        ...
        <footer className="carrier-ui-pagination">
        <p>Hiển thị <span className="font-medium">{pageStart} - {pageEnd}</span> trên <span className="font-medium">{totalOrders.toLocaleString('vi-VN')}</span> đơn hàng</p><div><button type="button" disabled={safePage === 1} onClick={() => setPage((value) => Math.max(1, value - 1))}><img src="/media/carrier-ui/chevron-left.svg" alt="Trang trước" /></button>{[1, 2, 3].filter((value) => value <= totalPages).map((value) => <button type="button" className={safePage === value ? 'is-current' : ''} key={value} onClick={() => setPage(value)}>{value}</button>)}{totalPages > 4 && <span>…</span>}{totalPages > 3 && <button type="button" onClick={() => setPage(totalPages)}>{totalPages}</button>}<button type="button" disabled={safePage === totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}><img src="/media/carrier-ui/chevron-right.svg" alt="Trang sau" /></button></div></footer>
      </section>
