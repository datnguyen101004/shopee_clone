...
function sellerInventoryEligibility(
  userId: string,
  variantId?: string,
...
export class InventoryService {
  constructor(
    ...
    private readonly expiryQueue: InventoryReservationQueueService,
  ) {}
  ...
  private async lockReservation(tx: Tx, reservationId: string): Promise<void> {
    await tx.$queryRaw(Prisma.sql`
      SELECT "id"
  ...
  private async sellerVariant(tx: Tx, userId: string, variantId: string) {
    const variant = await tx.productVariant.findFirst({
      where: sellerInventoryEligibility(userId, variantId),
  ...
  private mapBalance(row: InventoryRow | null, variant?: any): InventoryBalance {
    ...
    return {
      ...
      productImageUrl: cdnUrl ?? image?.url ?? null,
      variantName: item.name,
      sku: item.sku,
      lifecycle: item.status === 'ACTIVE' && !item.deletedAt ? 'active' : 'inactive',
      quantityOnHand: onHand,
      quantityReserved: reserved,
      quantitySold: sold,
      availableQuantity: onHand - reserved,
      lowStock: onHand - reserved <= INVENTORY_LOW_STOCK_THRESHOLD,
      version,
      updatedAt: (row?.updatedAt ?? item.updatedAt).toISOString(),
    };
  }
  ...
  async history(
    ...
    const items = await this.prisma.inventoryAdjustment.findMany({
      where: { variantId, ...(query.cursor ? { id: { lt: query.cursor } } : {}) },
  ...
  private mapAdjustment(item: any): InventoryAdjustment {
    return {
      ...
      reason: item.reason.toUpperCase() as InventoryAdjustmentReason,
      note: item.note ?? null,
      delta: item.delta,
      quantityOnHandBefore: item.quantityOnHandBefore,
      quantityOnHandAfter: item.quantityOnHandAfter,
      quantityReserved: item.quantityReserved,
      quantitySold: item.quantitySold,
      availableQuantity: item.quantityOnHandAfter - item.quantityReserved,
  ...
  async adjust(
    ...
    input: { delta: number; reason: InventoryAdjustmentReason; note: string | null },
  ): Promise<InventoryAdjustment> {
    const requestDigest = digest({ variantId, ...input });
    return this.withSerializableRetry(() =>
      this.prisma.$transaction(
        async (tx) =>
          this.adjustInTransaction(
            tx,
            userId,
            variantId,
            expectedVersion,
  ...
  async adjustInTransaction(
    ...
    if (prior) {
      if (prior.requestDigest !== requestDigest) throw new InventoryIdempotencyConflictError();
      return this.mapAdjustment(prior);
    }
    ...
    const current =
      ...
      (await tx.inventory.create({
        data: { variantId, quantityOnHand: 0, quantityReserved: 0, quantitySold: 0, version: 0 },
      }));
    ...
    const adjustment = await tx.inventoryAdjustment.create({
      data: {
        ...
        quantityOnHandAfter: next.quantityOnHand,
        quantityReserved: next.quantityReserved,
        quantitySold: next.quantitySold,
        inventoryVersion: next.version,
        idempotencyKey,
        requestDigest,
      },
    });
  ...
  async reserveForCheckout(
    ...
    const prior = await tx.inventoryReservation.findFirst({
    ...
    if (sorted.length === 0 || new Set(sorted.map((line) => line.variantId)).size !== sorted.length)
    ...
    const due = await tx.inventoryReservation.findMany({
      where: {
        ...
        lines: { some: { variantId: { in: sorted.map((line) => line.variantId) } } },
      },
      include: { lines: true },
    });
    ...
    for (const expired of due) {
      for (const line of [...expired.lines].sort((a, b) =>
    ...
    const reservation = await tx.inventoryReservation.create({
      data: {
        ...
        expiresAt,
        lines: {
          create: sorted.map((line) => ({ variantId: line.variantId, quantity: line.quantity })),
        },
      },
      include: { lines: true },
    });
    ...
    await this.expiryQueue.enqueue(
      tx,
      reservation.id,
      reservation.generationToken,
  ...
  async reacquireForPendingOrder(
    ...
    const previous = await tx.inventoryReservation.findUnique({
  ...
  async consumeInTransaction(
    ...
    if (reservation.status !== 'ACTIVE') {
      if (
        typeof reservation.terminalIdempotencyKey === 'string' &&
  ...
  async attachToPurchaseInTransaction(
    tx: Tx,
    reservationId: string,
    buyerId: string,
    purchaseId: string,
  ) {
    const attached = await tx.inventoryReservation.updateMany({
      where: { id: reservationId, buyerId, status: 'ACTIVE', purchaseId: null },
      data: { purchaseId },
    });
    if (attached.count === 1) {
      return tx.inventoryReservation.findUniqueOrThrow({ where: { id: reservationId } });
    }
    const existing = await tx.inventoryReservation.findFirst({
      where: { id: reservationId, buyerId, status: 'ACTIVE', purchaseId },
    });
    if (!existing) throw new InventoryIdempotencyConflictError();
    return existing;
  }

  async release(
    reservationId: string,
    reason: 'expired' | 'checkout-failed' | 'payment-failed' | 'released',
    idempotencyKey: string,
  ) {
    return this.withSerializableRetry(() =>
      this.prisma.$transaction(
        (tx) => this.releaseInTransaction(tx, reservationId, reason, idempotencyKey),
  ...
  async releaseInTransaction(
    ...
    if (!reservation) return reservation;
    if (reservation.status !== 'ACTIVE') {
      const expectedStatus = reason === 'expired' ? 'EXPIRED' : 'RELEASED';
      if (
        typeof reservation.terminalIdempotencyKey === 'string' &&
        (reservation.terminalIdempotencyKey !== idempotencyKey ||
  ...
  async expireDue(limit = 100) {
    ...
    const due = await this.prisma.inventoryReservation.findMany({
      where: { status: 'ACTIVE', expiresAt: { lte: databaseNow } },
      select: { id: true, generationToken: true },
      take: limit,
    });
    for (const reservation of due) {
      await this.release(reservation.id, 'expired', reservation.generationToken);
    }
    return due.length;
  }
}
