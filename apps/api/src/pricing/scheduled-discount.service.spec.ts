...
describe('ScheduledDiscountService', () => {
    ...
    const prisma = {
      shopDiscountProduct: {
        findMany: jest
          .fn()
          .mockResolvedValue([
            { productId: 'product-1', discountBasisPoints: 1250, campaignId: 'campaign-1' },
          ]),
      },
    };
    const service = new ScheduledDiscountService(prisma as never);
    const evaluatedAt = new Date('2026-08-19T00:00:00.000Z');
    const result = await service.resolveVariants(
      undefined,
      [{ id: 'variant-1', productId: 'product-1', priceMinor: 10001n, compareAtPriceMinor: null }],
      evaluatedAt,
    );
    expect(result.get('variant-1')).toMatchObject({
      basePriceMinor: 10001n,
      effectivePriceMinor: 8751n,
      discountBasisPoints: 1250,
      campaignId: 'campaign-1',
      evaluatedAt,
    });
    expect(prisma.shopDiscountProduct.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ productId: { in: ['product-1'] } }),
      }),
    );
  });
  ...
  it('does not create an invalid zero-priced result', async () => {
    const prisma = {
      shopDiscountProduct: {
        findMany: jest
          .fn()
          .mockResolvedValue([
            { productId: 'product-1', discountBasisPoints: 9000, campaignId: 'campaign-1' },
          ]),
      },
    };
    const service = new ScheduledDiscountService(prisma as never);
    const result = await service.resolveVariants(
      undefined,
      [{ id: 'variant-1', productId: 'product-1', priceMinor: 1n, compareAtPriceMinor: 10n }],
      new Date('2026-08-19T00:00:00.000Z'),
    );
    expect(result.get('variant-1')).toMatchObject({
      effectivePriceMinor: 1n,
  ...
  it('uses a half-open active window and preserves base pricing when no campaign matches', async () => {
      ...
      [
        {
          ...
          productId: 'product-1',
          priceMinor: 12345n,
          compareAtPriceMinor: 15000n,
        },
      ],
      evaluatedAt,
    );
    expect(result.get('variant-1')).toMatchObject({
      basePriceMinor: 12345n,
      effectivePriceMinor: 12345n,
      discountBasisPoints: 0,
      campaignId: null,
    });
    expect(prisma.shopDiscountProduct.findMany).toHaveBeenCalledWith(
    ...
    expect(prisma.shopDiscountProduct.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          campaign: expect.objectContaining({ isEnabled: true, archivedAt: null }),
        }),
      }),
    );
  });
    ...
    const prisma = {
      shopDiscountProduct: {
        findMany: jest
          .fn()
          .mockResolvedValue([
            { productId: 'product-1', discountBasisPoints: 3333, campaignId: 'campaign-1' },
          ]),
      },
    };
    const service = new ScheduledDiscountService(prisma as never);
  ...
  it('propagates PostgreSQL errors instead of treating them as no campaign', async () => {
    const failure = new Error('database unavailable');
    const prisma = { shopDiscountProduct: { findMany: jest.fn().mockRejectedValue(failure) } };
    const service = new ScheduledDiscountService(prisma as never);
    await expect(
      service.resolveVariants(
        undefined,
        [
          {
            id: 'variant-1',
