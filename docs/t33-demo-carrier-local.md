...
3. Chạy `docker compose --profile carrier up -d demo-carrier` để bật dịch vụ mô phỏng ở cổng 3010. Container này dùng cùng mạng và datasource PostgreSQL marketplace; không có database Carrier riêng.
...
Trạng thái hiển thị cho buyer/seller được lưu bền vững trong các bảng shipment, event, dispatch outbox và callback receipt của database marketplace. Bản thân tiến trình mô phỏng chỉ giữ bản sao remote-looking trong memory để mô phỏng HTTP; khởi động lại tiến trình không làm mất projection hoặc lịch sử đơn hàng trong database chính.
