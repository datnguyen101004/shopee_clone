        ...
        <section className="seller-product-detail__variants-section">
          ...
          <div className="seller-product-detail__table-wrap">
            <table className="seller-product-detail__table">
              <thead><tr><th className="management-table-id-cell">ID</th><th>Biến thể</th><th>SKU</th><th>Giá bán</th><th>Tồn kho</th><th>Phiên bản</th><th>Khối lượng</th></tr></thead>
              <tbody>{product.variants.map((variant) => <tr key={variant.id}><td className="management-table-id-cell">{variant.id}</td><td>{variant.combination.join(' · ') || 'Mặc định'}</td><td>{variant.sku}</td><td>₫{formatNumber(variant.priceMinor)}</td><td>{formatNumber(variant.stock)}</td><td>v{variant.inventoryVersion ?? 0}</td><td>{formatNumber(variant.weightGrams)} g</td></tr>)}</tbody>
            </table>
          </div>
        </section>
