...
const reviewInclude = {
  ...
  media: { select: mediaSelect, orderBy: [{ sortOrder: 'asc' }, { id: 'asc' }] },
...
function cursorDecode(
  ...
  try {
    ...
    throw new ReviewMediaError('invalid-review-cursor');
  }
}
...
export class ReviewsService {
  ...
  async stage(
    userId: string,
    upload: { buffer: Buffer; mimeType: string; width: number; height: number; storageKey: string },
  ) {
  ...
  async publicPage(
    productId: string,
    query: { rating: ReviewRating | null; limit: number; cursor: string | null },
  ): Promise<PublicProductReviewPage> {
    ...
    const product = await this.prisma.product.findFirst({
      ...
      select: { ratingAverageBasisPoints: true, ratingCount: true },
    });
    ...
    return {
      ...
      items: visible.map((review) => this.publicReview(review)),
      page: {
        limit: query.limit,
        rating: query.rating,
        nextCursor:
  ...
  async authorDetail(userId: string, reviewId: string): Promise<AuthorProductReview> {
    const review = await this.prisma.productReview.findFirst({
      where: { id: reviewId, buyerUserId: userId },
      include: reviewInclude,
    });
  ...
  async eligibility(
    ...
    const line = await this.prisma.orderLine.findFirst({
      where: { sourceCartLineId: orderLineId, order: { purchase: { buyerId: userId } } },
  ...
  async create(
    ...
    input: CreateProductReviewRequest,
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const prior = await tx.productReview.findFirst({
        where: { buyerUserId: userId, idempotencyKey },
        include: reviewInclude,
      });
      if (prior) {
        if (prior.requestDigest !== requestDigest) throw new ReviewIdempotencyConflictError();
        return this.authorReview(prior);
      }
      const line = await tx.orderLine.findFirst({
        where: {
          sourceCartLineId: orderLineId,
          orderId: orderReference,
          order: { status: 'DELIVERED', purchase: { buyerId: userId } },
        },
        include: { order: { select: { shopId: true } }, review: { select: { id: true } } },
      });
      ...
      if (mediaIds.length) {
        ...
        if (media.length !== mediaIds.length)
          throw new ReviewMediaError('review-media-unavailable');
      }
      ...
      for (const [sortOrder, id] of mediaIds.entries())
        await tx.reviewMedia.update({
          where: { id },
          data: { reviewId: review.id, state: 'ATTACHED', expiresAt: null, sortOrder },
        });
      await this.refreshAggregates(tx, review.productId, review.shopId);
      return this.authorReview(
        await tx.productReview.findUniqueOrThrow({
          where: { id: review.id },
          include: reviewInclude,
        }),
      );
  ...
  async update(
    ...
    input: UpdateProductReviewRequest,
  ): Promise<AuthorProductReview> {
    return this.prisma.$transaction(async (tx) => {
      ...
      const mediaIds = input.mediaIds ?? [];
      if (mediaIds.length) {
        const available = await tx.reviewMedia.findMany({
          where: {
            id: { in: mediaIds },
            uploaderId: userId,
            OR: [{ reviewId }, { state: 'STAGED', expiresAt: { gt: new Date() } }],
          },
          select: { id: true },
        });
        if (available.length !== mediaIds.length)
          throw new ReviewMediaError('review-media-unavailable');
      }
      ...
      for (const [sortOrder, id] of mediaIds.entries())
        await tx.reviewMedia.update({
          where: { id },
          data: { reviewId, state: 'ATTACHED', expiresAt: null, sortOrder },
        });
  ...
  async repairAggregates(): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      const products = await tx.productReview.findMany({
        select: { productId: true, shopId: true },
        distinct: ['productId', 'shopId'],
      });
  ...
  async adminExecuteReviewAction(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const newVisibility =
        action === 'HIDE' ? 'HIDDEN' : action === 'RESTORE' ? 'VISIBLE' : review.visibility;
      const now = new Date();
      ...
      if (action !== 'KEEP_VISIBLE') {
        await tx.productReview.update({
          where: { id: reviewId },
          data: { visibility: newVisibility, version: nextVersion },
        });
        await this.refreshAggregates(tx, review.productId, review.shopId);
      }
      await tx.sellerReviewReport.updateMany({
        where: { reviewId, status: 'OPEN' as any },
  ...
  private async refreshAggregates(
    tx: { productReview: any; product: any; shop: any },
    productId: string,
    shopId: string,
  ): Promise<void> {
    const [product, shop] = await Promise.all([
      tx.productReview.aggregate({
        where: { productId, visibility: 'VISIBLE' },
        _count: { _all: true },
        _avg: { rating: true },
      }),
      tx.productReview.aggregate({
        where: { shopId, visibility: 'VISIBLE' },
        _count: { _all: true },
        _avg: { rating: true },
      }),
    ]);
    await Promise.all([
      tx.product.update({
        where: { id: productId },
        data: {
          ratingCount: product._count._all,
          ratingAverageBasisPoints: Math.round((product._avg.rating ?? 0) * 100),
        },
      }),
      tx.shop.update({
        where: { id: shopId },
        data: {
          ratingCount: shop._count._all,
          ratingAverageBasisPoints: Math.round((shop._avg.rating ?? 0) * 100),
        },
      }),
    ]);
  }
  private publicReview(review: any) {
    return {
      id: review.id,
      rating: review.rating as ReviewRating,
      text: review.text,
      authorName: review.buyer.displayName,
      verifiedPurchase: true as const,
      media: review.media.map((media: any) => ({
        ...media,
        url: `/api/v1/review-media/${media.id}`,
      })),
      updatedAt: review.updatedAt.toISOString(),
    };
  }
  private authorReview(review: any): AuthorProductReview {
    return {
