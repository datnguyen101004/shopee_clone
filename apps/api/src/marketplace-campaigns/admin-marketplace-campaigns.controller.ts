import type { CampaignAdminPage, CampaignAdminParticipantPage, CampaignAdminSummary, CampaignBannerDetail, CampaignTypeSummary } from '@shopee-clone/contracts';
...
import { AdminCampaignPageQueryDto, AdminCampaignParticipantPageQueryDto, CampaignCancelDto, CampaignCursorQueryDto, CampaignPlacementDto, CampaignPreviewDto, CampaignPublishDto, CreateCampaignDto, UpdateCampaignDto } from './marketplace-campaigns.dto';
...
export class AdminMarketplaceCampaignsController {
  ...
  @Get() @Header('Cache-Control', 'private, no-store') @ApiOperation({ summary: 'List campaigns with page pagination, 10 items per page' }) list(@Query() query: AdminCampaignPageQueryDto): Promise<CampaignAdminPage> { return this.campaigns.listAdmin(query); }
  ...
  @Get(':campaignId/participant-details') @Header('Cache-Control', 'private, no-store') participantDetails(@Param('campaignId') id: string, @Query() query: AdminCampaignParticipantPageQueryDto): Promise<CampaignAdminParticipantPage> { return this.campaigns.participantDetails(id, query); }
