...
The initial eligibility score is: view `1`, favorite `3`, followed shop `3`, and valid order `5`; personalization requires score `>= 5`, a profile no older than 24 hours, and matching feature versions. These are configuration constants covered by tests, not token/session claims. Guests, stale profiles, and sparse profiles use baseline ranking.
