...
describe('pricing API client', () => {
  it('sends only the address/services and current cart ETag', async () => {
    const fetcher = vi.fn().mockResolvedValue(new Response(JSON.stringify(quote), { status: 200 }));
    await expect(
      getPricingQuote({ shippingAddressId: addressId, services: [] }, 2, fetcher),
    ).resolves.toEqual(quote);
    const init = fetcher.mock.calls[0]![1] as RequestInit;
    expect(init.headers).toMatchObject({ 'If-Match': '"cart-2"' });
    expect(init.body).toBe(JSON.stringify({ shippingAddressId: addressId, services: [] }));
